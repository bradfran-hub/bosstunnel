...
class CatalogueRefresh {
  ...
  async run() {
    ...
    const owners = this.graph.customerVaults?.unlockedOwners() || [];
    const rows = this.graph.sql(`SELECT r.* FROM CatalogueRefresh r JOIN Sources s ON s.id=r.source_id
      LEFT JOIN CustomerSources o ON o.source_id=s.id WHERE s.enabled=1 AND r.next_at<=?
      AND (o.customer_id IS NULL OR o.customer_id IN (SELECT value FROM json_each(?)))
      ORDER BY r.next_at,r.source_id LIMIT 100`).all(now, JSON.stringify(owners));
    const row = rows.find(item => !this.engine.sourceTasks.has(item.source_id));
    if (!row || this.stopped) return;
    const id = row.source_id;
    const access = this.graph.sourceAccess(id);
    ...
    try {
      ...
      try { access?.assertCurrent(); }
      catch (locked) {
        if (locked.status !== 423) throw locked;
        this.graph.sql("UPDATE CatalogueRefresh SET status='pending',next_at=? WHERE source_id=?").run(row.next_at, id);
        return;
      }
