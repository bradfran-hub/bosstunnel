...
This resource contract also applies to the authenticated Xtream `boss_api?action=playback` extension and Boss-aware M3U discovery. Standard Xtream movie/series/live and M3U playback routes resolve lazily and return HTTP 307 with the exact upstream Location and an empty body. GET, HEAD and Range are handled by the player/upstream; BOSS never fetches the media. Redirect-only outputs can select only candidates needing no custom headers. HTTP redirects cannot teach ordinary IPTV players provider headers; all-header-dependent candidates return 422 rather than being silently proxied. The compatibility addon returns all upstream URLs and its protocol-defined request-header hints. Those hints do not enable any BOSS proxy; support depends on the consuming client.
