...
Selected URLs are encrypted, revision-bound links that expire within 24 hours or earlier with upstream expiry. The bounded playback window permits ordinary seeking and HLS segment requests without an artificial five-minute movie cutoff; it does not extend an upstream resource's lifetime. Resolution caches retain their separate short, source-specific TTLs. Fetch fresh playback details before starting an expired choice or retrying a failed choice. Selecting one plays that exact resource without silently switching quality/provider; Automatic retains resolver fallback. Access, source revision, collection revision and media membership are rechecked on redemption. HLS child links are separately protected and respect the playback window and upstream expiry. No upstream credentials or required headers are exposed to the app.
