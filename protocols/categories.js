...
function record(graph, row) {
  const sourceName = typeof graph.source === "function" ? graph.source(row.source_id).name : row.source_name;
  return { id: String(row.id + 1), name: ["other", "catalogue", "boss"].includes(row.protocol) ? `${sourceName} | ${row.name}` : row.name, type: row.kind };
}
function primaryCategory(library, media, selected) {
  if (selected === "1") return { id: "1", name: "Boss Media", type: media.type };
  const row = library.graph.sql(`SELECT c.id,c.name,c.kind,c.source_id,s.name AS source_name,s.protocol
  ...
  return row ? record(library.graph, row) : { id: "1", name: "Boss Media", type: media.type };
}
function categoryPage(library, params) {
  ...
  const rows = library.graph.sql(`SELECT c.id,c.name,c.kind,c.source_id,s.name AS source_name,s.protocol
  ...
  const categories = [...(after === "0" ? [{ id: "1", name: "Boss Media", type }] : []), ...rows.map(row => record(library.graph, row))];
