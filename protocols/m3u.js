...
async function* xmltv(library) {
  const revision = library.collection.revision;
  ...
  while (true) {
    ...
    for (const event of events) {
      if (library.collection.revision !== revision) throw Object.assign(new Error("Library changed during guide export; retry the request"), { status: 409 });
      yield `<programme channel="${library.engine.synthetic("xtream", event.channel_id)}" start="${time(event.starts_at)}" stop="${time(event.ends_at)}"><title>${xml(event.title)}</title><desc>${xml(event.description)}</desc></programme>`;
    }
